...
step_funcao = lambda z:np.heaviside(z, 0)
step = FuncaoAtivacao(step_funcao,dz_funcao)
...
class RegressaoLogistica():
    ...
    def forward_propagation(self,mat_x):
        ...
        if(self.arr_w is None):
            self.arr_w = np.zeros(len(mat_x.T))
    ...
    def backward_propagation(self,arr_y):
        ...
        arr_dw = np.dot(arr_dz, self.mat_x) / len(arr_dz)

        #a partir de arr_dz, calcula db
        db = np.sum(arr_dz) / len(arr_dz)
    ...
    def loss_function(self,arr_y):
        ...
        """
        cross_entropy = 0
        for yi, ai in zip(arr_y, self.arr_a):
            # cross_entropy+=(yi*np.log(ai) + (1-yi)*np.log(1 - ai))
            cross_entropy += ai - yi

        # return -1 * cross_entropy / len(arr_y)
        return np.square(cross_entropy)
    ...
    def atualiza_pesos(self,learning_rate):
        ...
        """
        self.arr_w = self.arr_w - self.gradiente.arr_dw * learning_rate
        self.b = self.b - self.gradiente.db * learning_rate
